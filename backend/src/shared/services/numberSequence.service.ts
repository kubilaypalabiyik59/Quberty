...
import { FISCAL_YEAR_SCOPE } from './numberSequenceRules';
...
/**
 * The number-sequence RULES — highest-issued, format grammar, the automatic
 * resume decision and the counter range — live in `numberSequenceRules.ts`,
 * which imports nothing. They are re-exported here so callers that already
 * depend on this service keep one import, but the logic is deliberately not in
 * this file: it must be testable without pulling in Prisma.
 */
export {
  NEXT_NUMBER_MIN,
  NEXT_NUMBER_MAX,
  ...
  resumeBehindReason,
} from './numberSequenceRules';
export type { HighestIssued, FormatTokenReport, AutomaticResume } from './numberSequenceRules';
...
export async function allocateNumber(opts: AllocateOptions): Promise<string> {
  ...
  const resetsYearly = seq.scope === FISCAL_YEAR_SCOPE;
