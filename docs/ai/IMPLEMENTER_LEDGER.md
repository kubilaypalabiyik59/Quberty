...
| Landing T7 (e2e spec) | `docs/design/landing-tasks/T7.md`, `T7-fix1.md` | `d266ea2` | `ae4cf25`, `5f04802` (gate self-fix), `ff3cb28` | 2 | ACCEPTED 2026-09-25 | R1 model: 4 type errors (wrong import source, `for...of` over an object, `toHaveCSS` misuse), which the **gate fed back and the model fixed in the same run**, the gate's first live catch. It also wrote the glob `waitForURL('??lang=es')`, where `?` is a wildcard, because the architect described that step in prose, not code. R2: fixed with Playwright in the loop (`TEST_CMD`), and saw 17 pass itself. | The first task where the model wrote the code from behaviour descriptions rather than copying it. Result: 17/17 against the production build. |
| Landing T8 (mosaic field) | `docs/design/landing-tasks/T8.md`, `T8-fix1.md` | `5f86f53` | `1851e61`, `b532b54` (gate self-fix), `b3f8752` | 2 | ACCEPTED 2026-09-25 | R1 model: 5 type errors (missing `useEffect` import, variables scoped wrongly), fixed by the model from gate feedback. It also left 3 runtime defects the gate cannot see: the rAF loop died after the first frame (throttle `return` before rescheduling), the canvas was never sized (300x150 bitmap), and cleanup cancelled a stale frame id. | `mosaic.ts` (the algorithm from formulas in prose) was correct first time. Runtime logic still needs line-by-line review. The rAF pattern was added to the conventions. |
| Landing T9 (scroll stage) | `docs/design/landing-tasks/T9.md` | `8fb3fb1` | `f615995` | 1 | ACCEPTED 2026-09-25 | None. | Verified in the browser: `--p` goes 0 to 1 over the hero, `--q` tracks the page, and the cube glides to the centre. The sign-in page is visually unchanged. |
| Landing T10 (glass sections) | `docs/design/landing-tasks/T10.md` | `f615995` | `4dcc7d7` | 1 | ACCEPTED 2026-09-25 | None. | Mechanical 7-file class edit: 13/13 lines were exactly the specified transform, nothing extra. `11-landing.spec.ts` still passes 17/17. |
