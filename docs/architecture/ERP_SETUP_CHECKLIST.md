...
| ~~5.11~~ | ~~A `PURCHASE_EXPENSE` posting type~~ | **DONE 2026-08-16** — not-stocked purchases reach `PURCHASE_EXPENSE`, not COGS, on both the receipt and the invoice | — |
| ~~5.12~~ | ~~Separate physical from financial posting on the purchase side~~ | **DONE 2026-08-16** — product receipt and vendor invoice are separate documents; `post_physical_inventory`, `post_financial_inventory`, `accrue_liability_on_receipt` and `receiving_requirements` are all honoured. Three-way matching, price tolerances and invoice totals matching ship with it. Off by default per tenant. [VENDOR_INVOICE.md](../process/VENDOR_INVOICE.md) | — |
