    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowForm(true)}
          ...
          <Plus className="h-4 w-4" /> New Factura
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-xl border border-blue-200 shadow-sm p-6 mb-6">
          <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wider mb-4">New Factura</h2>
          ...
          <div className="flex gap-3">
            <button onClick={() => create.mutate()} disabled={!form.customer_name || !form.total_amount || create.isPending}
              ...
              <Check className="h-4 w-4" /> {create.isPending ? 'Creating...' : 'Issue Factura'}
            </button>
